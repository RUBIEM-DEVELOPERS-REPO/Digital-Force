...
What new Python function/skill would prevent these failures?
Respond with JSON:
{{
...
"""
    ...
    logger.info(f"[SkillForge] Forging new skill: {skill_name}")
...
"""
    ...
    if test_result.get("success"):
        ...
        # Save to database (via API call to avoid circular imports)
        logger.info(f"[SkillForge] ✅ Skill '{skill_name}' created and saved")

        return {
            "new_skills_created": new_skills + [skill_name],
            "messages": [{"role": "skillforge", "content": f"✨ New skill created: {skill_spec.get('display_name', skill_name)}"}],
            "next_agent": "monitor",
        }
    else:
        logger.warning(f"[SkillForge] Skill validation failed: {test_result.get('error')}")
        return {
            "messages": [{"role": "skillforge", "content": f"Skill forge attempted but validation failed: {test_result.get('error', 'unknown')}"}],
            "next_agent": "monitor",
        }
